...
from util import magenta, cyan, green, red, dechunk, rFile, wFile, cls, generate_file_mode
        ...
        if len(n) == 2:
            ...
            if F(n) == 0:
                ...
                if (not (s and s.strip())):
                    s = message_file_path
                file_mode = generate_file_mode(s)
                if len(file_mode) == 0:
                    print(red('Invalid file format!'))
                    os._exit(0)
                msg = rFile(s, file_mode[0])
                if ECB_CBC == 0:
                    cipher_str = cipher_threefish_msg(msg, MODE_ECB)
                    wFile(cipher_file_path, cipher_str, 0, file_mode[1])
                ...
                if ECB_CBC == 0:
                    ...
                    wFile(plain_text_file_path, plain_str, 5, file_mode[1])
